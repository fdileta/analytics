...
WHERE projects.namespace_id IS NULLSELECT COUNT(projects.id) FROM {{ref('gitlab_dotcom_projects_dedupe_source')}} AS projects INNER JOIN {{ref('gitlab_dotcom_namespaces_dedupe_source')}} AS namespaces ON projects.namespace_id = namespaces.custom_project_templates_group_id GROUP BY 1
