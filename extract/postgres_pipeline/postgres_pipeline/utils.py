...
from sqlalchemy.types import BigInteger, Integer, Text, Float, Text, Date, DateTime, Text
...
'STAR_COUNT':Integer(),
'MERGE_REQUESTS_REBASE_ENABLED':Text(),
'IMPORT_TYPE':Text(),
'IMPORT_SOURCE':Text(),
'APPROVALS_BEFORE_MERGE':Integer(),
'RESET_APPROVALS_ON_PUSH':Text(),
'MERGE_REQUESTS_FF_ONLY_ENABLED':Text(),
'ISSUES_TEMPLATE':Text(),
'MIRROR':Text(),
'MIRROR_USER_ID':Float(),
'SHARED_RUNNERS_ENABLED':Text(),
'BUILD_COVERAGE_REGEX':Text(),
'BUILD_ALLOW_GIT_FETCH':Text(),
'BUILD_TIMEOUT':Integer(),
'MIRROR_TRIGGER_BUILDS':Text(),
'PENDING_DELETE':Text(),
'PUBLIC_BUILDS':Text(),
'LAST_REPOSITORY_CHECK_FAILED':Text(),
'LAST_REPOSITORY_CHECK_AT':Text(),
'CONTAINER_REGISTRY_ENABLED':Text(),
'ONLY_ALLOW_MERGE_IF_PIPELINE_SUCCEEDS':Text(),
'HAS_EXTERNAL_ISSUE_TRACKER':Text(),
'REPOSITORY_STORAGE':Text(),
'REPOSITORY_READ_ONLY':Text(),
'REQUEST_ACCESS_ENABLED':Text(),
'HAS_EXTERNAL_WIKI':Text(),
'CI_CONFIG_PATH':Text(),
'LFS_ENABLED':Text(),
'ONLY_ALLOW_MERGE_IF_ALL_DISCUSSIONS_ARE_RESOLVED':Text(),
'REPOSITORY_SIZE_LIMIT':Text(),
'PRINTING_MERGE_REQUEST_LINK_ENABLED':Text(),
'AUTO_CANCEL_PENDING_PIPELINES':Integer(),
'SERVICE_DESK_ENABLED':Text(),
'DELETE_ERROR':Text(),
'LAST_REPOSITORY_UPDATED_AT':DateTime(),
'DISABLE_OVERRIDING_APPROVERS_PER_MERGE_REQUEST':Text(),
'STORAGE_VERSION':Float(),
'RESOLVE_OUTDATED_DIFF_DISCUSSIONS':Text(),
'REMOTE_MIRROR_AVAILABLE_OVERRIDDEN':Text(),
'ONLY_MIRROR_PROTECTED_BRANCHES':Text(),
'PULL_MIRROR_AVAILABLE_OVERRIDDEN':Text(),
'MIRROR_OVERWRITES_DIVERGED_BRANCHES':Text(),
...
def read_sql_tmpfile(query, db_engine, tmp_file):
    ...
    df = pd.read_csv(tmp_file, chunksize=500_000, low_memory=False)
    logging.info("CSV read")
    return df

def query_results_generator(
    query: str, engine: Engine, chunksize: int = 500_000
) -> pd.DataFrame:
    """
...
def range_generator(
    start: int, stop: int, step: int = 500_000
...
def get_db_metadata(
    ...
    logging.info(df[[ "column_name", "data_type"]])
